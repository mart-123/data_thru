...
def init():
    ...
    config['input_path'] = os.path.join(config['static_dir'], 'hesa_22056_ETHNICITY.csv')
...
def cleardown_sql_table(cursor):
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM load_hesa_static_22056_ethnicity")
        row_count = cursor.fetchone()[0]
        
        # Delete all rows from the table (commit logic is in 'main')
        cursor.execute("DELETE FROM load_hesa_static_22056_ethnicity")

        logging.info(f"Deleted {row_count} rows from load_hesa_static_22056_ethnicity")

    except Exception as e:
        logging.critical(f"Error clearing down SQL table load_hesa_static_22056_ethnicity: {e}")
        raise
...
def write_to_db_execute_many(csv_df: pd.DataFrame, cursor):
    ...
    try:
        ...
        insert_cmd = """
            INSERT  INTO load_hesa_static_22056_ethnicity
                        (code, label)
                    VALUES (%s, %s)
            """
