...
def read_in_chunks(conn, chunk_size=200):
    ...
    try:
        ...
        sql_select = """
                    SELECT t1.student_guid, t1.program_guid, t1.enrol_date, t1.fees_paid, t1.source_file, t1.hesa_delivery
...
def write_to_db_execute_many(input_df: pd.DataFrame, conn, config):
    ...
    try:
        ...
        input_cols = ['student_guid', 'program_guid', 'enrol_date', 'fees_paid',
        ...
        insert_cmd = f"""
            INSERT  INTO {config['output_table']}
                        (student_guid, program_guid, enrol_date, fees_paid,
                        source_file, hesa_delivery)
                    VALUES (%s, %s, %s, %s, %s, %s)
            """
